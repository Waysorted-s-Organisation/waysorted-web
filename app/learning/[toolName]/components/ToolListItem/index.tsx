...
function badgeToneClasses(tone: Tool["badge"] extends infer B ? B extends { tone: infer T } ? T : never : never) {
  switch (tone) {
    case "green":
      return "text-secondary-db-100 p-1 mx-2 bg-tertiary-green-500 text-white rounded-md";
    case "blue":
      return "text-secondary-db-100 p-1 mx-2 bg-primary-way-10 text-primary-way-100 rounded-md";
    case "gray":
    default:
      return "text-secondary-db-70 p-1 mx-2 bg-secondary-db-10 rounded-md";
  }
}
        ...
        <div>
          <h2 className="font-medium w-xs text-xl text-secondary-db-100 flex items-center">
            ...
            {(tool.badge?.label || tool.isNew) && (
              <span className={badgeToneClasses(tool.badge?.tone || (tool.isNew ? "green" : "gray"))}>
              </span>
            )}
          </h2>
